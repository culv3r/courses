...
Input:
    Book	same as Programming Assignment #5 although there may be different data.
    ...
    Command	This is is in the form of commands: 
        ...
        CUSTOMER TRANS cTransType    szBookId   szTransDt
            ...
            -	Lookup the book ID.  If not found, print a warning (but do not terminate your program) and return.
        ...
        BOOK PPRINT
            prints the binary tree of books in a pretty print style.
Results:
...
void processCommands(NodeT **ppRoot, char *pszCommandFileName);
...
{
    NodeT      *pRoot;                      // points to an ordered binary tree of books
    ...
    pRoot = getBooks(pszBookFileName);
    printBooks("Initial Books", pRoot);
    ...
    printBooks("Resulting Books", pRoot);
    printf("\n");
    return 0;
}
...
Parameters:
    I/O NodeT **ppRoot                Address of the pointer to the root of
                                      an ordered binary tree of books
    ...
    */
    while (fgets(szInputBuffer, 100, pfileCommand) != NULL)
    {
        ...
        {   // BOOK command
            bGotToken = getSimpleToken(szInputBuffer, &iBufferPosition, szSubCommand);
            if (bGotToken)
                processBookCommand(ppRoot, szSubCommand
...
/******************** getBooks **************************************
    NodeT *getBooks(char * pszBookFileName)
Purpose:
...
NodeT *getBooks(char * pszBookFileName)
{
    ...
    NodeT *pRoot= NULL;             // Pointer to the root of a binary tree
    ...
    */
    while (fgets(szInputBuffer, 100, pFileBook) != NULL)
    {
        ...
        // Insert the book into the binary Tree using the reconstruct
        // approach.  insertT will always return a pointer to
        // the referenced subtree.  For the first call, this will be
        // the root.  
        pRoot = insertT(pRoot, book);
    }
    fclose(pFileBook);
    return pRoot;
}
/******************** allocateNodeT **************************************
    NodeT * allocateNodeT(Book book)
Purpose:
...
void exitUsage(int iArg, char *pszMessage, char *pszDiagnosticInfo)
{
    if (iArg >= 0)
        ...
        , pszMessage
        , pszDiagnosticInfo);
    fprintf(stderr, "p5 -b bookFileName -c commandFileName\n");
    if (iArg >= 0)
        exit(-1);
    else
        exit(-2);
