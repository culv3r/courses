...
int main (int argc, char *argv[]){
    ...
    if ((fd1 != 0 && fd2 != 0) && argc < 4){
        ...
        while ((size = read(fd1, szBuf, sizeof szBuf)) > 0 || (size2 = read(fd2, szBuf2, sizeof szBuf2)) > 0){
            while(szBuf[x] != '\n' && x >= size){
                write(STDOUT_FILENO,szBuf[x],sizeof(szBuf[x]));
                x++;
            }
            write(STDOUT_FILENO,'\n',sizeof szBuf[x]);
            x++;
            while(szBuf2[y] != '\n' && y <= size2){
                write(STDOUT_FILENO,szBuf2[y],sizeof szBuf[x]);
                y++;
            }
            write(STDOUT_FILENO,'\n',sizeof szBuf[x]);
            y++
    ...
    if ((bInput1 && fd2 != 0) && argc < 4){
        ...
        while((size = read(STDIN_FILENO, szBuf, sizeof szBuf)) > 0 || (size2 = read(fd2, szBuf2, sizeof szBuf2)) > 0){
            if (szBuf[0] != '\0')
                write(STDOUT_FILENO, szBuf, size);
            while(szBuf2[y] != '\n' && y <= size2){
                write(STDOUT_FILENO, szBuf2[y], 1);
                y++;
            }
            write(STDOUT_FILENO, '\n', 1);
            y++;
    ...
    if ((fd1 != 0 && bInput2) && argc < 4){
        while((size = read(fd1, szBuf, sizeof szBuf)) > 0 && (size2 = read(STDIN_FILENO, szBuf2, sizeof szBuf2)) > 0){
            while(szBuf[x] != '\n' && x <= size){
                write(STDOUT_FILENO, szBuf[x], 1);
                x++;
            }
            write(STDOUT_FILENO, '\n', 0);
            x++;
            printf("[User Input Required, Please type your lines, when finished press CTRL + D]\n");
            if((size2 = read(STDIN_FILENO, szBuf2, sizeof szBuf2)) > 0){
